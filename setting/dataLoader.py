    ...
    def __getitem__(self, index):
        ...
        if(self.category=='train'):
            if random.random()<0.5:
                t = random.randint(1, 2)    #t不能为0 
                hsi=torch.flip(hsi,dims=[-t])
                Xdata=torch.flip(Xdata,dims=[-t])
                hsi_pca=torch.flip(hsi_pca,dims=[-t])
            # if random.random() < 0.5:
            #     angle = random.choice([90, 180, 270])
            #     hsi = torch.rot90(hsi, k=angle // 90, dims=[1, 2])
            #     Xdata = torch.rot90(Xdata, k=angle // 90, dims=[1, 2])
            #     hsi_pca = torch.rot90(hsi_pca, k=angle // 90, dims=[1, 2])            
