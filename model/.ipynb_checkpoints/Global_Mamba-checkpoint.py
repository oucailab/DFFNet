...
class Net(nn.Module):
    def __init__(self, dataset):
        ...
        self.model=VisionMamba(img_size = data['window_size'],channels = data['pca_num'],num_classes = data['num_classes'],
        patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True)

    def forward(self, hsi, sar):

        output_fusion = self.model(hsi)

        return 1, output_fusion
