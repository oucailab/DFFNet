...
class Syn_layer(nn.Module):
    def __init__(self,hc1,hc2,sc1,sc2,hsi_N,img_size,d_state,lay_n):
        super().__init__()
        N = 9 - lay_n * 2 #最初为0
        N1 = hsi_N - N + 1
        ...
        self.MBh = MSBlock(hidden_dim = hc2 * N1, d_state=d_state)
        self.SpBh = SpecMambaBlock(hidden_dim = (img_size-2)**2, d_state=d_state)
        self.MBx = MSBlock(hidden_dim=sc2, d_state=d_state)
        self.FSSBlock = FSSBlock(hidden_dim1=hc2*(hsi_N - N + 1), hidden_dim2=sc2, d_state=d_state)
    ...
    def forward(self, hsi, sar):
        ...
        # hsi = self.MBh(hsi) #只有空间Mamba,没有spectral
        sar = self.MBx(sar)
        out_hsi,out_sar = self.FSSBlock(hsi,sar)
        out_hsi,out_sar = hsi,sar
...
class Net(nn.Module):
    def __init__(self, dataset):
        ...
        # cha_h = [2,4,8, 16, 24, 32]
        # cha_x = [16,32,64, 128, 192, 256]
        cha_h = [8, 16, 24, 32]
        ...
        #获得一下有多少层
        self.layer_num = data['num_layer'] #得大于等于1
        self.layers = nn.ModuleList()
        for i in range(self.layer_num): 
            if(i==0):
                self.layers.append(Syn_layer(1,cha_h[i],sar_num,cha_x[i],pca_num,img_size,d_state,i))
            else:
                self.layers.append(Syn_layer(cha_h[i-1],cha_h[i],cha_x[i-1],cha_x[i],pca_num,img_size,d_state,i))
    ...
    def forward(self, hsi, sar):
        ...
        return fusion_feat_m,output_fusion   #需要将最后一层特征输出出来
