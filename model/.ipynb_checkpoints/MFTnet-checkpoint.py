...
sys.path.append("./../")
from torch.nn import LayerNorm, Linear, Dropout  # 保留实际使用的模块
import copy
...
class Net(nn.Module):
    def __init__(self, dataset):
        ...
        self.MFT = MFT(16, pca_num, slar_channel_num, num_classes, False,window_size)

    def forward(self, hsi, sar):
        # 移除多余的维度（如果存在）
        hsi = hsi.squeeze(dim=1)  # 假设输入是 [B,1,30,7,7] → [B,30,7,7]
        return None, self.MFT(hsi, sar)